...
\begin{table}[h]
  ...
  \end{table}
  % U is a better response than M if the probability of P2 and P3 both playing R at the same time is less than 0.4 (as then u_1(U)>6). Similarly, D is a better response than M if the probability of both P2 and P3 playing L is less than 0.4. But note that at least one of the two has to be true as P2 and P3 are mixing independently (each of the two players has to put at least 0.4 on each action but this implies that the product is less than 0.4). Hence M is a never best response.
 % A mix of U and D would be better than M against (L,L) if more than 0.6 probability was given on U. But then the mix does worse than M against (R,R). Hence, M is not strictly dominated. 
\item Show in the game depicted in table \ref{tab:mixNotRatio} that some mixed strategies are not rationalizable although all pure strategies are, i.e. show that some mixed strategies are never best responses while all pure strategies are best responses to some belief.
  ...
  \end{table}
  % U is b.r. against L, D against R and M against a 0.5-0.5 mix; L is br against U and R against M. Hence all pure strategies are rationalizable. Uniform mixing between U and D, however, is strictly dominated by M.
\item Suppose there are two players and let $S_i=[0,1]$. Assume that each player $i$ has a best response function, denoted by $b_i(s_j)$, which is (i) continuous, (ii) strictly increasing and (iii)  differentiable almost everywhere with derivative in $(0,\eta)$ for some $\eta<1$.
  ...
  \item Draw the best response functions in a graph and use the graph to show that the unique Nash equilibrium actions are the unique rationalizable actions of the players.
    ...
    \end{enumerate}
    % a: $s_1$ ($s_2$) on horizontal (vertical axis): $br_2$ connects left right borders of unit box with slope less than 1, $br_1$ connects bottom top borders with slope more than 1; iterate set of possible best responses and note that in each step the length of the interval is at most $\eta<1$ times the length previously
   % b: best response function -> unique b.r.: $u_i$ strictly (quasi-)concave in $s_i$; continuous b.r.: $u_i$ continuous in $s_i$ and $s_{-i}$ ; strictly increasing: cross derivative of u_i strictly positive (use IFT); derivative <1: d u_i/ds_i reacts stronger on s_i than in s_{-i} (IFT)
  \item Show that the order of elimination does not matter for the outcome of iterative elimination of never best responses. \\(Hint: Consider two orders, say A and B, and suppose they lead to different outcomes. Consider then the first action to be deleted in one procedure, say A, while surviving the other, i.e. B. Show that this action cannot be a best response at the end of procedure B which yields the desired contradiction.)
    % let $s_i'$ be the first action eliminated in A that is not eliminated in B. As s' is not eliminated at the end of B, it is a br against some $s_{-i}'$ that survive B. As $s_i'$ is the first such action all actions of other players that survive elimination under B are still available at the point of time where $s'$ is eliminated in A and therefore some action $s_i''\neq s_i'$ must be the best response to $s_{-i}'$ (if all actions in $S_i$ are allowed). But as $s_{-i}'$ survives B, $s_i''$ cannot be eliminated at any step in B. But this contradicts that $s_i'$ is a best response to $s_{-i}'$ at the end of B.
...
\item (Numerical) Consider two players participating in a lab experiment. The monetary payoffs the two players get from the experimenter when choosing their strategies are given in table \ref{tab:monRatio}. (note: this time the numbers in the table are not utilities but amounts of money!) Assume that players are only interested in their own monetary payoffs and not in the monetary payoffs for the other players.
  ...
  \item Suppose players value money  $u_i(x)=-e^{-r x}$ for any monetary amount $x$. Which actions are rationalizable if the parameter $r$ is very high? (you can choose a high number like $3$ and check)
     ...
     % a) M1 is strictly dominated by a uniform mixing of U and D. As M2 is a br to a uniform mixing between L and R all remaining actions are rationalizable.
     % b) now M1 is rationalizable as it is a b.r. to uniform mixing of L and R
     % c) now M" is no longer rationalizable as it is strictly dominated by a uniform mixing between U and D
\end{enumerate}
...
% best response exists by Weierstrass theorem and is unique by strict quasi-concavity in s_i; the best response is determined implicitly by the unique solution to $\partial u_i/\partial s_i=0$ and as the RHS is continuous in both variables the implicitly defined function is continuous; drawing the (s_1, s_2) box, b_1 connects top and bottom while b_2 connects left and right boundary; as both are continuous they must intersect.  
\item Take the Cournot model from the slides but assume that $P(q_1+q_2)=1-q_1-q_2$ and let the two firms have different marginal costs. Namely assume that firm 1 has marginal costs of $1/4$ and firm 2 has marginal costs of $1/2$.
  ...
  \item Why does the proof on the slides not work if firms do not have the same marginal costs?
    ...
    \end{enumerate}
    % a)  foc: 1-2q_i-q_j-c_i=0  which gives the solution  q_1=1/3, q_2=1/12
    % b) no symmetric NE as firms have different br functions; hence fixed point of one br function is not useful
    % c) use exercise 1; note that this does not ensure uniqueness
\item Take a two player game with $S_i=[0,1]$. Assume that each player has a best response function that is differentiable with derivative in $(0,\eta)$ for some $\eta<1$. Show that there can be at most one pure strategy equilibrium in this game.
  % suppose 2 NE: (s_1',s_2') and (s_1'',s_2''); then |s_i''-s_i'|=|b_i(s_j'')-b_i(s_j')|\leq \eta |s_j''-s_j'|<|s_j''-s_j'| but this cannot hold true both for i=1, j=2$ and for $i=2,j=1.
\item Take a finite strategic form game $\Gamma=\langle I,\{S_i\},\{u_i\} \rangle$ and let $S_i$ be binary, i.e. $S_i=\{L,R\}$. Now suppose that each player is restricted to mixed strategies that place at least probability $\varepsilon >0$ on each of his two actions (e.g. the player ``trembles'' sometimes and even if he wants to choose $L$, he will choose $R$ with probability $\varepsilon $ and vice versa).
  ...
  \item Adapt the proof of the Nash theorem to show that there is a Nash equilibrium in this game  with ``trembles''.
    ...
    % b) adjust strategy set and min, max expression to ensure g and g map into $[\varepsilon ,1-\varepsilon ]$
    % c) u_i(L,L)=1 and 0 otherwise; then (R,R) is NE in game without trembles but not in tremble game
\end{enumerate}
...
\item Two player Cournot with uncertain costs: Payoffs of player $i$ are $(\theta _i-q_1-q_2)q_i$ where $\theta _i$ is the difference between the intercept of a linear demand function and costs. It is commonly known that $\theta _1=1$. However, $\theta _2$ is private information of firm 2. Firm 1's (commonly known) belief is that $\theta _2$ is $3/4$ with probability $1/2$ and $5/4$ with probability $1/2$.
  ...
  \item If type $j\in\{l,h\}$ of firm 2 produces quantity $q_2^j$, what is firm 1's best response quantity? (i.e. maximizing expected payoff)
    \item Determine the BNE of this game.
    \end{enumerate}
  ...
  % strategy is mapping from type to price; in symmetric BNE price $p^ĥ$ if high costs and $p^l$ if low costs; best response is $arg\max_{p} 0.5*(1-p-p^h/2)*(p-c)+0.5*(1-p-p^l/2)(p-c)$; the foc gives $1-2p-p^h/4-p^l/4+c=0$ and therefore $p^h$ and $p^l$ solve $1-2p^h-p^h/4-p^l/4+0.5=0$ and $1-2p^l-p^h/4-p^l/4=0$. The solution is $p^h=5/8$ and $p^l=3/8$  
...
\label{tab:purify}
\end{table}
% P1's expected utility of playing U is $u^U_1=\theta _1+5*Prob[\theta _2>\bar\theta _2]$ while $u^D_1=1+3*Prob[\theta _2>\bar\theta _2]$. As $Prob[\theta _2>\bar\theta _2]=1-\bar{\theta}_2/\varepsilon $, the indifference condition that has to hold at the cutoff type $\bar{\theta }_1$ yields $\bar\theta _1=2\bar{\theta }_2/\varepsilon -1$. Similarly, $u_2^L=\theta _2+1+3Prob[\theta _1<\bar{\theta }_2]$ and $u_2^R=5Prob[\theta _1<\bar{\theta }_2]$. Using $Prob[\theta _1<\bar{\theta }_2]=\bar{\theta }_1/\varepsilon $, the indifference condition for $\bar{\theta }_2$ yields then $\bar{\theta }_2=2\bar{\theta }_1/\varepsilon -1$. Solving the two conditions for $\bar{\theta }_1$ and $\bar{\theta }_2$ gives the solution $\bar{\theta }_1=\bar{\theta }_2=\varepsilon /(2-\varepsilon )$. The probability of each action therefore converges to $1/2$ as $\varepsilon \rightarrow 0$. This is equivalent to the mixed strategy equilibrium of the game in which $\theta _1=\theta _2=0$ is common knowledge. 
\item Consider the game in table \ref{tab:global}. Assume that $x$ is an unknown state of nature that is distributed uniformly on $[-1,5]$. Each of the two players observes a private signal $\theta _i$. The signals are drawn by nature independently from a uniform distribution on $[x-0.1 ,x+0.1 ]$). To summarize: Each player only observes his signal and knows neither the other player's signal nor the state $x$. However, the distributions that generate state and signals are common knowledge.  
  \begin{enumerate}
  \item Determine a player's belief about the state after receiving signal $\theta _i$.
    % uniform on $[\theta _i-0.1,\theta _i+0.1]\cap[-1,5]$
  \item Which action will a player take if he observes a signal above 4.1 (or below $-0.1)$?
    % as x is definitely above 4 (below 0), H (S) is dominant
  \item Determine a Bayesian Nash equilibrium of the game.\footnote{This setup was introduced as ``global game'' in \cite{carlsson1993global} to analyze equilibrium selection in coordination games. A survey of the early literature on this topic that covers the main mechanics is \cite{morris2001global}.}\\ (\emph{Hints:} Treat the signals as types and a strategy as a mapping from signal to action. Players use a cutoff strategy in equilibrium.)
    % let equilibrium be symmetric; if one receives signal equal to cutoff, one has to be indifferent between actions and it is equally that other player gets signal above or below cutoff (i.e. belief for each of opponent's actions has to be 1/2); hence cutoff is 2. 
  \end{enumerate}
      \begin{table}[h]
\centering
 \begin{game}{2}{2}
